...
if (Sys.info()['sysname'] != "Windows") {
   occsPaleo <- try(occs_paleoDb(spName, occNum, timeInterval, logger = NULL), silent = TRUE)
}
...
test_that("error checks", {
  ...
  skip_if(class(occsPaleo) == "try-error")
  ...
  #              fixed = TRUE)
  #BAJ 6/25/2024: this kept failing so I took it out & replaced it with skip_if == "try-error"
})
