...
occs_paleoDb <- function(spName, occNum, timeInterval, logger = NULL) {
  ...
  if (nrow(occsOrig) == 0 | ncol(occsOrig) == 0) {
    logger %>% writeLog(
      type = "error",
      hlSpp(fmtSpN(spName)),
      "The PBDB API returned no records for this query. Please check the spelling or change the interval.")
    return()
  }
  ...
  occsOrig$accepted_name <- as.character(occsOrig$accepted_name)
  names(occsOrig)[names(occsOrig) == "accepted_name"] <- "scientific_name"
  ...
  # subset by key columns and make id and popup columns
    #BAJ 6/25/2024: early_interval, late_interval, early_age, late_age not available for all spp
